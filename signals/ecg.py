...
class SimpleECG(nn.Module):
    """
    A tiny 1D Convolutional Neural Network (CNN)
    used to classify a single-lead ECG segment as Normal or Abnormal.
    """

    def __init__(self, input_length=5000):
        super().__init__()
        
        # Store the expected input length (number of ECG samples per segment)
        # The model always expects 5000 samples at a time.
        ...
        self.conv_net = nn.Sequential(
            ...
            # - Input channels: 1 (single ECG lead)
            # - Output channels: 16 (feature maps)
            # - Kernel size: 5 → each filter covers 5 time points
            # - Padding = 2 → keeps the same temporal length after convolution
            nn.Conv1d(1, 16, 5, padding=2),
            
            # ReLU activation adds non-linearity
            nn.ReLU(),
            
            # Max pooling reduces the temporal resolution by half (downsampling)
            nn.MaxPool1d(2),
            
            # Second convolution layer:
            # - Takes 16 input channels and produces 32 output feature maps
            # - Same kernel size and padding
            nn.Conv1d(16, 32, 5, padding=2),
            
            nn.ReLU(),
            # Another MaxPool1d halves the time dimension again
            nn.MaxPool1d(2),
        )
        
        # --------------------------
        # Dynamically compute the input size for the linear layer
        # We just finished the convolution layers .
 #Now, we want to connect those features to fully connected (linear) layers that make the final decision (Normal / Abnormal).
        # --------------------------
        with torch.no_grad():
            # Create a dummy input of zeros with the same expected length (5000 samples)
            dummy_input = torch.zeros(1, 1, input_length)
            
            # Pass it through the convolutional network
            dummy_output = self.conv_net(dummy_input)
            
            # Count the total number of elements (to flatten into a vector)
        ...
        self.classifier = nn.Sequential(
            # Flatten the convolutional output to a 1D vector
            nn.Flatten(),
            
            # First fully connected layer:
            # - Input: whatever size came from the conv output
            # - Output: 64 neurons (hidden representation)
            #The resulting 3D tensor (batch × channels × length) is flattened into a 1D vector.
            #That vector is passed into this Linear layer.
            #This layer transforms it into 64 new values
        
            nn.Linear(linear_input_size, 64),
            #Activation function that keeps only positive values and adds non-linearity
            nn.ReLU(),
            
            # Final output layer:
            # - 64 → 2
            # - The 2 outputs correspond to [Normal, Abnormal]
            nn.Linear(64, 2)
        )
def forward(self, x):
    """
    Defines how data moves through the model for prediction.
    Input: x = ECG signal of shape (batch_size, 1, 5000)
    Output: raw class scores (logits) for [Normal, Abnormal]
    """

    # Step 1: Pass the input through the convolutional network (self.conv_net)
    # - This is the feature extraction part of the model.
    # - Layers inside conv_net include:
    #       Conv1d: Detects local ECG patterns (like QRS complexes)
    #       ReLU: Adds non-linearity so the model can learn complex shapes
    #       MaxPool1d: Reduces signal length (downsamples) while keeping key features
    # - After this step, 'x' is no longer a raw ECG waveform — it becomes
    #   a set of learned features that represent important signal characteristics.
    x = self.conv_net(x)

    # Step 2: Pass the extracted features into the classifier (self.classifier)
    # - This is the decision-making part of the model.
    # - The classifier includes:
    #       Flatten(): Turns all features into one long vector
    #       Linear → ReLU → Linear: Fully connected layers that
    #         combine and transform features into a decision space.
    # - Essentially, it learns to answer:
    #       “Given these patterns, is the ECG Normal or Abnormal?”
    # - The final Linear layer outputs 2 numbers — one for each class.
    x = self.classifier(x)

    #  Step 3: Return the output logits
    # - 'x' now contains raw scores (before applying softmax).
    # - Example: tensor([[2.3, -1.1]])
    #       → Class 0 (Normal): score = 2.3
    #       → Class 1 (Abnormal): score = -1.1
    # - Higher score = more likely.
    # - Softmax is applied later (outside this function) to convert these
    #   scores into probabilities [p(Normal), p(Abnormal)].
    return x
...
# ------------------------------------
#  This creates an instance of the SimpleECG class defined earlier.
#  _model_seq_len (usually 5000) is the number of samples the model expects for each ECG segment.
#  .to(DEVICE) moves the model to either CPU or GPU, depending on availability.
#  Think of this as building an empty "skeleton" of the model — the structure (layers) exists,
#   but the model doesn’t yet know anything (weights are not loaded yet).
model = SimpleECG(input_length=_model_seq_len).to(DEVICE)



#  SIMPLE_MODEL_PATH is the path to the saved model weights (like "simple_ecg_model.pt").
#  This line checks if that file exists on disk.
#   If it exists → load the saved parameters (learned weights).
#   If not → skip to using an untrained model for demonstration.
if os.path.exists(SIMPLE_MODEL_PATH):
    try:
        
        # torch.load(...) reads the saved model weights file from disk.
        # map_location=DEVICE ensures the weights load correctly to the same hardware (CPU/GPU).
        ...
        # ----------------------
        # If loading was successful, log an info message confirming that weights were loaded.
        logger.info("Loaded 1D model from %s", SIMPLE_MODEL_PATH)

    except Exception as e:
        
        # -------------------------------------------------
        # - If loading fails (e.g., file corrupted, incompatible weights, wrong architecture),
        #   the exception is caught here.
        # - The program will NOT crash — instead, a warning message is logged.
        logger.warning("Failed to load 1D model: %s", e)

else:
    #  No pretrained weights found
    # --------------------------------------
    # - If the file doesn’t exist at all, it means there are no learned weights available.
    # - The model will remain untrained  using random initial weights.
    #   This mode is useful for demonstrations, showing how the system works,
    #   but predictions will not be meaningful.
    logger.info("simple_ecg_model.pt not found — using untrained 1D model (for demo).")


#   Set model to evaluation mode
# ----------------------------------------
# - This tells PyTorch that the model is being used for inference (not training).
# - Ensures consistent, stable outputs during prediction.
model.eval()
