...
def save_audio_file(signal, fs, duration, min_fs=8000):
    ...
    sig_audio = resample_signal(signal, fs, audio_fs, method="linear") if audio_fs != fs else signal
...
def resample_uploaded_audio():
    try:
        ...
        y_resampled = resample_signal(y_orig, sr_orig, target_fs, method="linear") if target_fs != sr_orig else y_orig
        
        # Save for playback (minimum 8kHz)
        audio_fs = max(8000, target_fs)
        y_audio = resample_signal(y_resampled, target_fs, audio_fs, method="linear") if audio_fs != target_fs else y_resampled
